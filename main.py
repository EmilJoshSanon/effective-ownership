...
# NOTES:
# 1. All companies related to focus company are connected in a graph
...
def populate_effective_ownership(
    ...
    for n in range(len(network)):
        if (
            network[n].target_name == target_company
            and network[n].real_lower_share is None
        ) and network[n].active:
            # Extract numerical upper and lower share from share string and calculate average share.
            ...
            if previous_node:
                ...
                result_network[n].real_lower_share = (
                ...
                result_network[n].real_upper_share = (
            ...
            # Multiply the three different shares (lower, average, and upper) with the effective ownership from the current_node and add it the network.
            # Save network entry into current_node
            # Recursively call calculate_effective_ownership with the new current_node
            result_network = populate_effective_ownership(
                network[n].source_name, result_network, network[n]
            )
